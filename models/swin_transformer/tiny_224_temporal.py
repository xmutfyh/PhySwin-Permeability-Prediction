...
# 时序模型（PhyTemporalHead），其余操作/参数与单帧配置对齐：Grayscale + Normalize，自动统计，无回退
import numpy as np
import pandas as pd
...
def _compute_k_stats_from_excel(label_path: str, column: str = "k", offset: float = LOG_OFFSET):
    """计算 log(k+offset) 的均值/标准差（总体标准差，ddof=0）"""
    try:
...
print(f"[tiny_224_temporal] computed k stats from {LABEL_PATH}: mean={k_mean:.6f}, std={k_std:.6f}")
print(f"[tiny_224_temporal] computed tvp stats from {LABEL_PATH}: means={tvp_means}, stds={tvp_stds}")

# ====== 模型配置（与单帧一致的骨干/颈部/优化器设置；仅 head 为时序） ======
model_cfg = dict(
    backbone=dict(
    ...
    head=dict(
        ...
        tcn_layers=3,
        dropout=0.1,
        num_classes=2,  # stage 分类
        loss=dict(type='HuberLoss', delta=1.0),
        reg_loss_weight=1.0,
        cls_loss_weight=1.0,
        monotonic_cfg=dict(
            enabled=True, weight=0.005, eps=0.02, margin=0.0,
            cond_order=['T', 'V', 'P'],
...
########################
data_cfg = dict(
    batch_size=4,   # 与单帧对齐；如显存不足，可降低并用 accumulate_steps 提升有效批次
    num_workers=2,
    seq_len=8,       # 仅新增的时序窗口
