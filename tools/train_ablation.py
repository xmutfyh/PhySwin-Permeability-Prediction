...
from utils.train_utils_ablation import train, validation, print_info, file2dict, init_random_seed, set_random_seed, resume_model, \
    test
...
# ------------------------------------------------------------
# Reproducibility helpers (Windows-safe for DataLoader workers)
# NOTE: worker_init_fn must be a top-level function to be picklable
# ------------------------------------------------------------
def seed_worker(worker_id: int):
    """
    Seed numpy/random in each DataLoader worker process.
    Uses torch.initial_seed() which is derived from the DataLoader's generator.
    This is the recommended, deterministic approach for multiprocessing.
    """
...
def parse_args():
    ...
    parser.add_argument('--seed', type=int, default=42, help='random seed (default: 42 for ablation reproducibility)')
    ...
    parser.add_argument('--tvp-mode', default='TVP', choices=['TVP','TP','TV','T'],
                        help='TVP ablation mode: TVP(all), TP(zero V), TV(zero P), T(zero V and P)')
    parser.add_argument('--cond-only', action='store_true', help='Zero-out images to remove visual information (TVP-only ablation)')
...
def main():
    ...
    save_dir = os.path.join('logs', model_cfg.get('backbone').get('type'), f"{dirname}_tvp-{args.tvp_mode}")
    ...
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=data_cfg.get('batch_size'),
                              ...
                              collate_fn=collate, worker_init_fn=seed_worker, generator=g)
    val_loader = DataLoader(val_dataset, shuffle=False, batch_size=data_cfg.get('batch_size'),
                            ...
                            drop_last=True, collate_fn=collate, worker_init_fn=seed_worker, generator=g)
    test_loader = DataLoader(test_dataset, shuffle=False, batch_size=data_cfg.get('batch_size'),
                            num_workers=data_cfg.get('num_workers'), pin_memory=True,
                            drop_last=False, collate_fn=collate, worker_init_fn=seed_worker, generator=g)
