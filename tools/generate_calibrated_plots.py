...
# 改动：
# - 移除校准/标定逻辑
# - 输出对齐时序脚本：预测CSV、对比CSV、图像产物（kt_pred/kt_pred_gt/kt_seq、散点、误差散点、误差直方图）
# - 列命名统一：filename, seq_id, t, index, pred_k_std, pred_k；可选 T,V,P；真值列名为 k
# 默认输出（按你的要求）：
# - 使用TVP：outputs/infer_TVP/infer_YYYYMMDD_HHMMSS.csv
# - 不使用TVP：outputs/infer_noTVP/infer_YYYYMMDD_HHMMSS.csv
# - 对比CSV同目录：infer_vs_right_YYYYMMDD_HHMMSS.csv
# 说明：
# - 静态脚本不做时间窗口聚合/时滞搜索，不输出阶段概率或相变时刻误差图
# ------------------------------------------------------------
import os
import sys
import glob
...
def _fallback_error_plots(y_true, y_pred, path_scatter, path_hist):
    """
    误差诊断图：
    - 使用绝对误差 |pred - gt|，与“Absolute Error”标签一致
    - path_scatter：绝对误差随样本 index 的散点图
    - path_hist：绝对误差直方图
    """
    if y_true.size == 0 or y_pred.size == 0:
        return
    ...
    plt.title('Absolute Error Scatter')
    plt.tight_layout()
...
def build_meta_from_dirs(infer_dir, right_dir, use_tvp=True):
    infer_excels = glob.glob(os.path.join(infer_dir, "*.xlsx")) + \
                   glob.glob(os.path.join(infer_dir, "*.xls"))
    if not infer_excels:
    ...
    right_excels = glob.glob(os.path.join(right_dir, "*.xlsx")) + \
                   glob.glob(os.path.join(right_dir, "*.xls"))
    if not right_excels:
    ...
    right_df.columns = ["index", "k"]  # 统一命名为 'k'
    ...
    meta = img_df.merge(infer_df, on="index", how="left") \
                 .merge(right_df, on="index", how="left")
...
def parse_args():
    ap = argparse.ArgumentParser(description="Static single-frame inference (outputs aligned with temporal script)")
    ap.add_argument("config")
    ap.add_argument("--ckpt", required=True)
    ap.add_argument("--infer-dir", required=True)
    ap.add_argument("--right-dir", required=True)
    ap.add_argument("--use-tvp", action="store_true")
    ap.add_argument("--out-csv", default=None, help="输出预测CSV路径（默认写入 outputs/infer_TVP 或 outputs/infer_noTVP）")
    ap.add_argument("--out-compare", default=None, help="输出对比CSV路径（默认同目录）")
...
def main():
    ...
    # 根据是否使用 TVP 选择默认目录
    base_out_dir = 'outputs/infer_TVP' if args.use_tvp else 'outputs/infer_noTVP'
    ...
    # 图3：最终每序列曲线（kt_seq*.png）
    try:
        _plot_k_t_curves(out_pred, right_df=r_for_plot, out_dir=out_dir, prefix='kt_seq')
    except Exception as e:
        print('[Warn] plot final kt_seq failed:', e)
