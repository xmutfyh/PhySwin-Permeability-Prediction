...
from core.initialize import constant_init
...
class BasicBlock(BaseModule):
    """BasicBlock for a ResNet-like architecture.

    Args:
        in_channels (int): Input channels of this block.
        out_channels (int): Output channels of this block.
        expansion (int): Reserved argument in BasicBlock and should always be 1. Default: 1.
        stride (int): stride of the block. Default: 1
        dilation (int): dilation of convolution. Default: 1
        downsample (nn.Module, optional): downsample operation on identity branch. Default: None.
        style (str): `pytorch` or `caffe`. It is unused and reserved for unified API with Bottleneck.
    ...
    def __init__(self,
        ...
        self.conv1 = build_conv_layer(conv_cfg, in_channels, self.mid_channels, 3,
                                      stride=stride, padding=dilation, dilation=dilation, bias=False)
        self.add_module(self.norm1_name, norm1)
        self.conv2 = build_conv_layer(conv_cfg, self.mid_channels, out_channels, 3,
...
class Bottleneck(BaseModule):
    """Bottleneck block for a ResNet-like architecture.
    ...
    def __init__(self,
        ...
        self.conv1 = build_conv_layer(conv_cfg, in_channels, self.mid_channels, kernel_size=1,
                                      stride=self.conv1_stride, bias=False)
        self.add_module(self.norm1_name, norm1)
        self.conv2 = build_conv_layer(conv_cfg, self.mid_channels, self.mid_channels, kernel_size=3,
...
class ResLayer(nn.Sequential):
    ...
    def __init__(self,
        ...
        self.drop_rate = drop_rate  # 保存 drop_rate 参数
        ...
        for i in range(1, num_blocks):
            layers.append(
                block(
                    ...
                    **kwargs))
        if self.drop_rate > 0:
            layers.append(nn.Dropout(p=self.drop_rate))  # 在 ResLayer 中添加 dropout
...
class ResNet(BaseModule):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            # print(f"Output of layer {i} ({layer_name}): {x.shape}")
            if self.drop_rate > 0:  # 在每个 ResLayer 之后应用 dropout
                x = nn.Dropout(p=self.drop_rate)(x)
            if i in self.out_indices:
                outs.append(x)
        # print(f"Final output from backbone: {outs}")
        return tuple(outs)
